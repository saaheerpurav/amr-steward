...
AMR-Steward is an OpenEnv-compliant reinforcement learning environment in which an LLM agent
(Qwen3-4B + LoRA, trained with GRPO) learns to prescribe the correct antibiotic for
...
| Base LLM | Qwen3-4B + LoRA (r=16) |
| Environment framework | OpenEnv (Meta-PyTorch, Gymnasium-style) |
...
│
├── env/                      # Core OpenEnv environment package
...
│
├── test_env.py               # OpenEnv integration smoke tests (8 tests)
